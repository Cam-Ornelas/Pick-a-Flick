...
// Hide genres menu.
let selectedRatings = document.getElementById("selectedRatings");
selectedRatings.style.display = "none";
...
form.addEventListener("submit", (e)=>{
    ...
    if (genresObject[input.trim().toUpperCase(input)]){
        ...
        //Call function.
        searchedFor.style.fontSize = "50px";
        searchedFor.innerHTML = "Rating: " + "<span>" +input+ "</span>";
			...
			for(let i = 0; i < movie.length; i++){
				...
				if(favoriteMovies.indexOf(id) === -1){
					...
					<div class="card">
						...
						<div class="movie">
							<h2>${movie[i].title}</h2>
								<p id="p_rating"><strong>Rating:</strong> <span>${movie[i].vote_average} / 10  <i class="fa fa-star" style="font-size:24px; color:gold"></i></span> </p>
								<p><strong>Release date:</strong> <span>${movie[i].release_date} <i class="fa fa-calendar"></i> </span></p>
...
// MOVIES BY RATING
function discoverMovies(rating){
    //Show the select genres.
    selectedGenres.style.display = "block";

    //Reset the "selected" option on form submit.
    selectedGenres.selectedIndex = 0;
    sessionStorage.removeItem("movieByRatingGenre");
    sessionStorage.setItem("rating", rating)

    axios.get("https://api.themoviedb.org/3/discover/movie?api_key="+API_KEY+'&language=en-US&sort_by=popularity.desc&page=1&vote_average.gte='+rating)
        .then((response)=>{
            let movie = response.data.results;
            let output = "";
            for(let i = 0; i < movie.length; i++){
            ...
            // MOVIE BY RATING WITH GENRE:
            
            selectedGenres.addEventListener("change", (e)=>{
                pageNum = 1;
                const selectedGenre = e.target.options[e.target.selectedIndex].id;
                sessionStorage.setItem("movieByRatingGenre", selectedGenre);
                axios.get("https://api.themoviedb.org/3/discover/movie?api_key="+API_KEY+'&language=en-US&sort_by=popularity.desc&page=1&vote_average.gte='+rating+'&with_genres='+selectedGenre)
                .then((response)=>{
                let movie = response.data.results;
                let output = "";
                for(let i = 0; i < movie.length; i++){
...
function movieByRatingPage(pageNum){
    let rating = sessionStorage.getItem("rating");
    let genre = sessionStorage.getItem("movieByRatingGenre")
    if(!genre || !genre.length){
        genre = "";
    }
    axios.get('https://api.themoviedb.org/3/discover/movie?api_key='+API_KEY+'&language=en-US&sort_by=popularity.desc&page='+pageNum+'&vote_average.gte='+rating+'&with_genres='+genre)
    .then((response)=>{
        let movie = response.data.results;
        let output = "";
        for(let i = 0; i < movie.length; i++){
